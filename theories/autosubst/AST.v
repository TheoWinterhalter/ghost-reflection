...
Lemma congr_Pi {s0 : mode} {s1 : term} {s2 : term} {t0 : mode} {t1 : term}
  {t2 : term} (H0 : s0 = t0) (H1 : s1 = t1) (H2 : s2 = t2) :
  Pi s0 s1 s2 = Pi t0 t1 t2.
Proof.
exact (eq_trans
         (eq_trans (eq_trans eq_refl (ap (fun x => Pi x s1 s2) H0))
            (ap (fun x => Pi t0 x s2) H1)) (ap (fun x => Pi t0 t1 x) H2)).
Qed.
...
Fixpoint ren_term (xi_term : nat -> nat) (s : term) {struct s} : term :=
  ...
  | Pi s0 s1 s2 =>
      Pi s0 (ren_term xi_term s1) (ren_term (upRen_term_term xi_term) s2)
  | lam s0 s1 =>
...
term :=
  ...
  | Sort s0 s1 => Sort s0 s1
  | Pi s0 s1 s2 =>
      Pi s0 (subst_term sigma_term s1)
...
subst_term sigma_term s = s :=
  ...
  | Pi s0 s1 s2 =>
      congr_Pi (eq_refl s0) (idSubst_term sigma_term Eq_term s1)
...
ren_term xi_term s = ren_term zeta_term s :=
  ...
  | Sort s0 s1 => congr_Sort (eq_refl s0) (eq_refl s1)
  | Pi s0 s1 s2 =>
      congr_Pi (eq_refl s0) (extRen_term xi_term zeta_term Eq_term s1)
...
subst_term sigma_term s = subst_term tau_term s :=
  ...
  | Sort s0 s1 => congr_Sort (eq_refl s0) (eq_refl s1)
  | Pi s0 s1 s2 =>
      congr_Pi (eq_refl s0) (ext_term sigma_term tau_term Eq_term s1)
...
{struct s} : ren_term xi_term s = subst_term sigma_term s :=
  ...
  | Sort s0 s1 => congr_Sort (eq_refl s0) (eq_refl s1)
  | Pi s0 s1 s2 =>
      congr_Pi (eq_refl s0) (rinst_inst_term xi_term sigma_term Eq_term s1)
